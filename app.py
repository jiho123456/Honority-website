...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        ...
        c.executemany(
            "INSERT INTO class_schedule (time_range, monday, tuesday, wednesday, thursday, friday) VALUES (?,?,?,?,?,?)",
            default_schedule
        )
    ...
    if c.fetchone()[0] == 0:
        ...
        c.executemany(
            "INSERT INTO learning_resources (title, url, description) VALUES (?,?,?)",
            default_resources
        )
    conn.commit()
    return conn
...
# ---------------------------
    ...
    with st.form("chat_form", clear_on_submit=True):
        ...
        if submitted and nickname and message:
            ...
            c = conn.cursor()
            c.execute(
                "INSERT INTO chat_messages (nickname, message, timestamp) VALUES (?,?,?)",
    ...
    if chat_rows:
        for nick, msg, ts in reversed(chat_rows):
            st.markdown(f"**[{ts}] {nick}**: {msg}")
    else:
...
# ---------------------------
    ...
    with st.form("blog_form", clear_on_submit=True):
        ...
        if submitted and title and content:
            ...
            c = conn.cursor()
            c.execute(
                "INSERT INTO blog_posts (title, content, timestamp, username) VALUES (?,?,?,?)",
    ...
    c = conn.cursor()
    c.execute(
        "SELECT id, title, content, timestamp, username FROM blog_posts ORDER BY id DESC"
    )
    blog_data = c.fetchall()
    if blog_data:
        for post_id, title, content, ts, author in blog_data:
            st.markdown(f"**[{post_id}] {title}** _(작성일: {ts}, 작성자: {author})_")
            st.write(content)
            if st.session_state.logged_in and st.session_state.role in ["제작자", "관리자", "헌재"]:
                if st.button(f"삭제 (ID {post_id})", key=f"delete_{post_id}"):
                    d = conn.cursor()
                    d.execute("DELETE FROM blog_posts WHERE id=?", (post_id,))
                    conn.commit()
...
# ---------------------------
    ...
    with st.form("homework_form", clear_on_submit=True):
        ...
        if submitted_hw and hw_title and hw_desc and hw_due:
            now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            c.execute(
                "INSERT INTO homework (title, description, due_date, timestamp, username) VALUES (?,?,?,?,?)",
    ...
    c = conn.cursor()
    c.execute(
        "SELECT id, title, description, due_date, timestamp, username FROM homework ORDER BY id DESC"
    )
    hw_data = c.fetchall()
    if hw_data:
        for hw_id, hw_title, hw_desc, due_date, ts, author in hw_data:
...
# ---------------------------
    ...
    if st.session_state.logged_in and st.session_state.role in ["제작자", "관리자"]:
        with st.expander("편집 모드: 학습 자료 관리"):
            ...
            d = conn.cursor()
            d.execute("SELECT id, title FROM learning_resources")
            for res_id, res_title in d.fetchall():
                if st.button(f"삭제 (ID {res_id})", key=f"del_res_{res_id}"):
                    dd = conn.cursor()
                    dd.execute("DELETE FROM learning_resources WHERE id=?", (res_id,))
                    conn.commit()
                    st.success("삭제됨")
                    st.rerun()
    if resources:
        ...
        with st.expander("편집 모드: 단어 추가/삭제"):
            with st.form("add_word", clear_on_submit=True):
                ...
                if submitted_word and new_word and new_def and new_ex:
                    c.execute(
                        "INSERT INTO word_of_the_day (word, definition, example) VALUES (?,?,?)",
            ...
            d = conn.cursor()
            d.execute("SELECT id, word FROM word_of_the_day")
            for word_id, word in d.fetchall():
                if st.button(f"삭제 (ID {word_id}: {word})", key=f"del_word_{word_id}"):
                    dd = conn.cursor()
                    dd.execute("DELETE FROM word_of_the_day WHERE id=?", (word_id,))
                    conn.commit()
                    st.success("삭제됨")
                    st.rerun()
...
# ---------------------------
    ...
    if st.session_state.logged_in and st.session_state.role in ["제작자", "관리자"]:
        with st.expander("편집 모드: 수업 일정 추가/삭제"):
            ...
            d = conn.cursor()
            d.execute("SELECT id, time_range FROM class_schedule")
            for sched_id, time_range in d.fetchall():
                if st.button(f"삭제 (ID {sched_id}: {time_range})", key=f"del_sched_{sched_id}"):
                    dd = conn.cursor()
                    dd.execute("DELETE FROM class_schedule WHERE id=?", (sched_id,))
                    conn.commit()
                    st.success("삭제됨")
                    st.rerun()
    c.execute(
        "SELECT time_range, monday, tuesday, wednesday, thursday, friday FROM class_schedule ORDER BY id ASC"
    )
    schedule_rows = c.fetchall()
    if schedule_rows:
        schedule_df = pd.DataFrame(
            schedule_rows,
            columns=["시간", "월요일", "화요일", "수요일", "목요일", "금요일"]
        )
